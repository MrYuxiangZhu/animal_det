...
def load_tiny_detector_pro(cfg, device):
    train_cfg = cfg["tiny_detector_pro"]
...
def postprocess_tiny_pro(raw_outputs, anchors, conf_threshold: float, iou_threshold: float, image_size: int, original_shape: Tuple[int, int], scale: float, pad: Tuple[int, int]):
...
def build_tiny_pro_frame_inferencer(model, anchors, cfg, device, tracker=None, source=""):
    ...
    def infer_frame(frame):
        ...
        detections = postprocess_tiny_pro(raw_outputs, anchors, cfg["infer"]["conf_threshold"], cfg["infer"]["iou_threshold"], image_size, frame.shape[:2], scale, pad)
